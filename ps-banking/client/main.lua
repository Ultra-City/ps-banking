...
Citizen.CreateThread(function()
    ...
    for _, location in pairs(Config.BankLocations.Coords) do
        ...
        if Config.TargetSystem == "DrawMarker" then
            local textUI = {}
            local coords = vector3(location.x, location.y, location.z)
            local pointbank = lib.points.new({
                coords = coords,
                distance = Config.drawmarkerDistance
            })
            function pointbank:nearby()
                if self.currentDistance < self.distance then
                    DrawMarker(Config.drawmarkerType, self.coords.x, self.coords.y + 1.0, self.coords.z, 0.0, 0.0, 0.0,
                        0.0, 180.0, 0.0, 0.4, 0.4, 0.4, 245, 147, 66, 150, true, false, 2, false, nil, nil, false)
                    if not textUI then
                        lib.showTextUI(locale("openBank"), {
...
Citizen.CreateThread(function()
    if Config.TargetSystem == "DrawMarker" then
        while true do
            Wait(2500)
            for i = 1, #Config.ATM_Models do
                local ped = PlayerPedId()
                local pedCoords = GetEntityCoords(ped)
                local atm = GetClosestObjectOfType(pedCoords.x, pedCoords.y, pedCoords.z, 15.0, Config.ATM_Models[i],
                    false, false, false)
                local atmCoords = GetEntityCoords(atm)
                local textUI = {}
                NetworkRegisterEntityAsNetworked(atm)
                local pointAtm = lib.points.new({
                    coords = atmCoords,
                    distance = Config.drawmarkerDistance
                })
                function pointAtm:nearby()
                    if self.currentDistance < self.distance then
                        DrawMarker(Config.drawmarkerType, self.coords.x, self.coords.y + 0.5, self.coords.z + 0.5, 0.0,
                            0.0, 0.0, 0.0, 180.0, 0.0, 0.4, 0.4, 0.4, 245, 147, 66, 150, true, false, 2, false, nil,
                            nil, false)
                        if not textUI then
                            lib.showTextUI(locale("openBank"), {
                                icon = "fa-solid fa-piggy-bank"
